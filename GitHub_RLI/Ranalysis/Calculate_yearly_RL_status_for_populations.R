...
#--------------------------------------------------------
##--------------------------------------------------------
#This script calculates a continuous Red List Index for oceanic predatory fishes between 1950 and 2019 using the estimated extinction risk of the 18 species of tunas, billfishes, and sharks. We applied the IUCN Criteria A "population reduction" to calculate the extinction risk (Red List  Threat Status) for 18 species of tunas, billfishes, and sharks.  Criterion A measures the extinction risk based on exceeding thresholds of population decline over a time frame of three generation lengths (or a minimum of 10 years, whichever is longer). 
...
setwd(dirname(rstudioapi::getActiveDocumentContext()$path)) # it sucks this does not work. So I need to set my WD below:
setwd("/Users/mjuanjorda/Dropbox/POSTDOC/MarieCurieProject/Redlistindex/RLI_january2015/RLI_2020/Data_code_sharing/Ranalysis")
...
#This look for each population estimates the average annual rate of change over the prior 3GL yearly, then calculates the total percent change over the 3GL yearly, and then A1 or A2 thresholds are applied for assigning a Red List Category 
...
for(spp in 1:length(unique(datos_diff$stockid))){  
  ...
  ## This look calculates (1) average annual rate of change over 3GL (2) total percent change over 3GL and (3)   assigns Red List Category for each time step (yearly)
  ...
  number_modelruns<-length(unique(as.numeric(factor(data_subset_by_y$ModelRun))))
    ...
    #model_output <- sampling(model1, data = data.list,chains=3, iter=12000, warmup=3000, thin=10,control = list(adapt_delta = 0.8),cores=6)
  ...
  #yearfirst=rep(firstyear,length(tpercent_change3GL))
  yearlast=rep(lastyear,length(tpercent_change3GL))
  #AverageAnnualRC=AverageAnnualRC
  #tpercent_change3GL=tpercent_change3GL
  ...
  table_save2<-rbind(table_save2,table_save) # this would go to save individual file ouputs for each stock
...
path <- "/Users/mjuanjorda/Dropbox/POSTDOC/MarieCurieProject/Redlistindex/RLI_january2015/RLI_2020/Data_code_sharing/output"
write.csv(table_save2,file.path(path, filename))
...
setwd("/Users/mjuanjorda/Dropbox/POSTDOC/MarieCurieProject/Redlistindex/RLI_january2015/RLI_2020/Data_code_sharing/Ranalysis") #BORRAR AL FINAL
...
library(dplyr)
  ...
  example2$perc<-as.numeric(example2)
...
##-------------------------------------------------------------
## The output folder contains yearly RL status for each population. Read and combine all the population into one data frame in preparation of the RLI calculation
...
# STEP6 calculates the Red List Index of populations (including 52 populations of tunas, billfishes and sharks)
  #-----------------------------------------------------
  #load(file="../output/Yearly_RL_status_populations.RData")
  ...
  summary(threat_data)
  source("/Users/mjuanjorda/Dropbox/POSTDOC/MarieCurieProject/Redlistindex/RLI_january2015/RLI_2020/Data_code_sharing/Ranalysis/RLI_population.R")
